...
- 1.在主库上把数据更改记录到二进制日志中（这些记录称为`二进制事件`）
...
然后可以使用`SHOW SLAVE STATUS`来查看复制是否正确执行
...
从主库读取一个二进制事件是一个`阻塞型网络调用`，当主库记录事件后，马上就开始发送，因此速度很快
...
可用性可以定义为服务器以`足够好的性能`处理请求的`时间段`（排除预热等准备时间）

## 实现高可用性

通过两种方法：`避免宕机`以及`加速宕机后的恢复`

策略是在系统中制造冗余，并具备故障转移能力

有两个度量：平均失效时间（MTBF，两次股掌之间的正常运行时间）和平均恢复时间（MTTR，从故障中恢复的时间）

## MySQL同步复制

使用同步复制时，主库上的事务只有在`至少一个备库上提交`后才能认为其执行完成

## 故障转移和故障恢复

假设服务器B作为服务器A荣誉的备份，故障转移指的是当A发生故障时，`用B来替代它`，而故障恢复指的是在修复了A之后可以再`替换回来`

对于拥有故障恢复功能的系统，可以认为故障转移包含了替代和替换回来两个步骤
