...
- 8.标志字段（6位），包含一些标志位：ACK（应答）、RST、SYN、FIN（用于连接建立和拆除）、CWR、ECE（用于明确拥塞通告）、PSH（指示接收方将数据交付上层）、URG（指示发送端的`被上层指示为紧急`的数据） 标志字段和首部长度字段有一些保留位，它们三个`合在一起有16位`
- 9.紧急数据指针字段（16位
...
- 1.客户向服务器发送一个不包含数据的报文段，其中的`SYN标志位置1`，并且随机生成一个`初始序号`（client_isn），放置于报文段的`序号字段`中。这个特殊报文段被称为`SYN报文段`，注意只有第一次握手时的`ACK为0`，其余的报文ACK都为1
...
假如一个家庭网络的IP地址为10.0.0.0/24，它们都属于这个专用网络中`具有专用地址的地域`，即仅对该网络中的设备有意义

可能除了当前的家庭网络外，还有许多的家庭网络都使用了10.0.0.0/24这个网络空间，那么在外部网络传输的分组就不能使用这些地址。而专用网络中的主机也需要与外部的互联网通信，这时就需要进行`网络地址转换（Network Address Translation NAT）`

NAT通常是由`NAT使能路由器`来实现的，它对于外界来说就像是一个具有`单一IP地址`的`单一设备`
...
如同上面的模型所示，NAT使能路由器对外界`隐藏`了家庭网络的细节，它通过`NAT转换表`来决定将到达的分组转发给哪个内部主机

家庭网络中的主机访问外部服务器的过程如下：

- 1.主机10.0.0.1通过一个端口2345（随机）将数据报发送到LAN中

- 2.NAT路由器收到数据报，并为该数据报`生成一个新的源端口号`5001（随机），将`源IP地址替换`为NAT使能路由器的广域网一侧IP地址138.76.29.7，同时在NAT转换表中`增加一个表项`

- 3.外部服务器收到分组，返回响应报文，目的地址是`NAT路由器的IP地址`168.76.29.7，目的端口号是5001

- 4.响应报文被NAT路由器收到后，根据目的IP地址与端口号从NAT转换表中`查找`出对应的主机10.0.0.0:2345，`重写数据报`并向其转发

注意到NAT虽然运行在网络层，但执行与路由器十分不同的功能，可以将它视为一个`中间盒`

### IPv6

随着IP节点与子网数量的增长，32比特的IPv4地址已经被用尽，于是为了增大地址空间，开发了`128比特`的IPv6协议

#### IPv6数据报格式

IPv6数据报的首部长度为`40字节`，有以下几个字段：

- 1.版本号，4比特

- 2.流量类型，8比特，与IPv4的服务类型相似

- 3.流标签，20比特，用于表示一条数据报的`流`，某些流可能是发送方想要被特殊处理的，如一种非默认服务质量或需要实时服务的流，例域音频和视频传输

- 4.有效载荷长度，16比特，以`字节`为单位给出数据长度

- 5.下一个首部，8比特，与IPv4中上层协议字段类似，指示该数据报应该被交付给哪个`运输层协议`

- 6.跳限制，类似IPv4中的`寿命`，每经过一个路由器该值减1，为0时该数据报被丢弃

- 7.源地址和目的地址，各128比特

#### IPv4到IPv6的迁移

IPv6的使能系统可以做成`向后兼容`，即支持发送、路由和接收IPv4数据报，但已经部署的IPv4系统却不能处理IPV6数据报

目前广泛使用的方法是`建隧道`，将两台IPV6路由器之间的IPV4路由器集合称为一个`隧道`

在隧道发送端的IPv6节点将`整个IPv6数据报`放到一个IPv4数据报的`数据字段`中，通过一系列IPv4路由器传到隧道接收端的IPv6节点，它可以从中取出IPv6数据报，然后再为IPv6数据报提供路由

## 通用转发和SDN

为了统一越来越多的中间盒应用（如NAT，防火墙，深度分组检测以及负载均衡器等），需要有一中通用的`匹配加动作`范式

其中匹配指的是对协议栈的多个`首部字段`进行匹配，动作包括转发分组到一个或多个端口，跨越多个通向服务的离开接口进行负载均衡（负载均衡器），重写首部值（NAT），有意识地阻挡或丢弃某个分组（防火墙）以及为进一步处理而向某个特定的服务器发送一个分组（DPI）等

通用转发中一个比较成熟的标准是`OpenFlow`，与路由器的基于目的地转发类似，它通过`远程控制器`，来控制一个匹配加动作转发表，称为`流表`

流表中有三个表项：

- 1.首部字段值的集合，用于与入分组匹配，匹配不上流表项的分组将被丢弃或发送到远程控制器做更多处理

- 2.计数器集合，包括已经与该表项匹配的`分组数量`，以及自从该表项上次更新以来的`时间`

- 3.动作集合，包括转发等动作

### 匹配

流表中的匹配字段通常包含一个`入端口号`和若干个`分组首部字段`，而且分组首部字段可能包含链路层、网络层和运输层`三个层次`的首部字段

入端口指的是该分组交换机上输入端口

分组交换机的工作流程可以理解为：将从某端口输入的、符合流表项中匹配条件的分组执行对应的动作

### 动作

最重要的动作有三个：`转发`、`丢弃`和`修改字段`


# 网络层：控制平面

转发表或流表有两种控制方式：`每路由器控制`和`逻辑集中控制`

## 路由选择算法

路由选择算法的目的是从发送方到接收方选择一条最低开销的路径

路由选择算法有几种分类方式：

- 1.根据集中还是分散来划分

    - 集中式路由选择算法，以所有节点之间的连通性以及所有链路的开销为输入，需要知道关于连通性和链路开销方面的`完整信息`，也称为`链路状态算法（Link State LS）`

    - 分散式路由选择算法，路由器以迭代、分布式的方式计算出最低开销路径，每个节点仅有与其`直接相连`的链路的开销信息就可以开始计算，也称为`距离向量算法（Distance-Vector DC）`

- 2.根据动态还是静态划分

    - 静态路由选择算法，路由随时间的变化缓慢，通常人工调整

    - 动态路由选择算法，随着网络流量负载或拓扑发生变化而改变路由选择路径

- 3.根据负载敏感还是负载迟钝划分

    - 负载敏感算法，链路开销会动态变化以反映链路拥塞程度

    - 负载迟钝算法，链路开销不明确的反映拥塞程度，当前的因特网路由选择算法都是`负载迟钝`的

### 链路状态路由选择算法

在链路状态算法中，网络拓扑和所有的链路开销都是已知的，这是通过让每个节点向网络中`所有`其他节点`广播链路状态分组`来完成的

常用的链路状态算法有Dijkstra算法（图论中的最小路径算法），它是一种迭代算法，通过k次迭代可以得到k个目的节点的最低开销路径

如果所有路由器都同时使用LS算法计算最短路径的话，有可能上次计算和下次计算得到的是完全相反的路径，这样就产生了`振荡`，所以通常路由器们在不通过时刻执行LS算法（这一部分详细内容请看书中P248）

### 距离向量路由选择算法

距离向量是一种`迭代的`、`异步的`和`分布式`的算法，每个节点都要从一个或多个直接相连的邻居接收信息，执行计算，然后将计算结果发送给邻居，而此过程要一直持续到邻居之间`无更多信息`要交换为止，且不要求所有节点相互步伐一致地操作

在DV算法中，应用了重要的Bellman-Ford公式：

```
d_x(y) = min_v {c(x, v) + d_v(y)}
```

其中d_x(y)是节点x到节点y地最低开销路径的开销，c(x, v)表示`相邻`的节点x和v之间的开销

DV算法的基本思想为：每个节点x以D_x(y)开始，对所有节点y，`估计`从x到y的最低开销，令`Dx = [D_x(y): y∈N]`是节点x的`距离向量`，该向量是从x到在N中的所有其他节点y的`开销估计向量`，每个节点x维护以下路由选择信息：

- 1.对于x的每个邻居v，开销为c(x, v)

- 2.节点x的距离向量，即`Dx = [D_x(y): y∈N]`

- 3.每个邻居的距离向量，对每个邻居v，有`Dv = [D_v(y): y∈N]`

在DV算法中，每个节点不时地向每个邻居发送`距离向量副本`，而当节点x从邻居v接收到一个新的距离向量，它首先`保存`v的距离向量，然后使用Bellman-Ford公式`更新`自己的距离向量：

```
D_x(y) = min_v {c(x, v) + D_v(y)}
```

如果x的距离向量`改变了`，就需要向每个邻居`发送`改变后的距离向量，按照这种方式，最终开销估计D_x(y)会收敛到d_x(y)

这样的过程一直持续到`没有更新报文发送`为止，此时算法停留在静止状态，直到一条`链路开销发生改变`

在LS算法中，节点需要先获得网络的完整信息再开始执行算法，而在DV算法中，节点只具有与邻居相关的信息

- 注意，为了获得转发表，节点x需要知道的不是到y的最短路径距离，而是沿着最短路径到y的`下一跳路由器邻居节点v(y)`

*这一部分示例请参看书P251*

#### 链路开销改变与链路故障

运行DV算法的节点检测到从它自己到邻居的链路开销发生`变化`时，就更新其距离向量，而且如果`最低开销路径`的开销发生了变化，就要向邻居`通知`最新的距离向量

但是在最低开销路径发生变化的时候，有可能出现`路由选择环路`（书P252），计算出新的最低开销路径的过程可能要迭代多次，称为`无穷计数`问题

可以通过`毒性逆转`的方法解决这个问题：如果z通过y路由到目的地x，那么z将通告y，它到x的距离是`无穷大`，这样就避免了y去计算包含通过z到达目的地x的路径，避免了环路

#### LS与DV算法的比较

- 1.报文复杂性，因为LS要求每个节点知道网络的完整信息，所以当一条链路开销改变时，必须向所有节点发送新的链路开销，所以LS的报文复杂性要高于DV

- 2.收敛速度，DV算法收敛较慢

- 3.健壮性，因为LS节点仅计算自己的转发表，某种程度上路由计算是`分离`的，所以健壮性较好

## 自治系统的内部路由选择：OSPF

随着路由器的数量剧增，不可能实现所有路由器之间互相广播报文，所以将整个因特网划分为一个个块，称为`自治系统（Autonomous System AS）`，在一个自治系统内部运行的路由选择算法称为`自治系统内部路由选择协议`

### 开放最短路优先 OSPF

Open Shortest Path First OSPF是一种`LS协议`，每个路由器具有完整的自治系统拓扑图，并向自治系统内所有其他路由器广播路由选择信息

信息包含在OSPF报文中，直接由IP承载

OSPF能够鉴别路由器，保证安全性，并可以同时使用多条相同开销的路径，综合支持单播与多播路由选择，也支持在单个AS中的层次结构


## ISP之间的路由选择：BGP

当跨越多个AS进行路由选择时，需要一个`自治系统间路由选择协议`，在因特网中，所有AS都使用`边界网关协议（Broder Gateway Protocol BGP）`，它是一种`分布式`、`异步`的协议

### BGP的作用

在BGP中，分组不是路由到一个特定的目的地址，而是路由到CIDR化的`前缀`，前缀表示一个子网或子网的集合，比如一个目的地可以是138.16.68/22的形式，它包括1024个IP地址

因此路由器的表项为`(x, I)`，其中x是一个前缀，I是路由器的一个接口号
...
对于每个AS有两种路由器，位于AS边缘的`网关路由器`和位于AS内部的`内部路由器`

在BGP中。每对路由器通过使用`179`端口的`半永久`TCP连接交换路由选择信息，每条连接以及所有通过该连接发送的BGP报文，称为`BGP连接`，跨越两个AS的BGP连接称为外部BGP(eBGP)连接，在AS中的BGP连接称为内部BGP(iBGP)连接

### 确定最好的路由

当路由器通过BGP连接通告前缀时，它在前缀中包括一些`BGP属性`，前缀及其属性称为`路由`

其中两个比较重要的属性是AS-PATH和NEXT-HOP

- AS-PATH 包含了通告已经通过的AS列表，每当一个前缀通过某AS时，该AS就将自己加入AS-PATH现有的列表

使用AS-PATH属性可以检测和防止通告环路，如果一台路由器在AS-PATH中看到了包含自己的AS，它将`拒绝`该通告

- NEXT-HOP 是AS-PATH`起始路由器接口`的`IP地址`

#### 热土豆路由选择

在热土豆路由选择中，对于一个路由器，它会选择在`自己AS中`，自身到NEXT-TOP路由器有最小开销的那条路径，而`忽略`在AS之外的其他部分


#### 路由器选择算法

进入BGP的路由选择算法的输入是到某前缀的所有路由的`集合`，按照下面的规则从中选出一条路由：

- 1.路由被指派一个`本地偏好`属性，他可能由本地路由器设置或在相同AS中的另一台路由器学习到，选择其中具有`最高本地偏好值`的路由

- 2.如果有多个，将在其中选择具有`最短AS-PATH`的路由，如果该规则是唯一规则，BGP将使用`距离向量算法`决定路径，且距离测度使用`AS跳数`而不是路由器条数

- 3.如果还是有多个，使用热土豆算法，选择`最靠近NEXT-HOP`路由器的路由

- 4.如果仍然有多个，则使用`BGP标识符`来选择

### IP任播

BGP还常用于实现IP任播服务，它可以：

- 1.在许多分散的不同地理位置，`替换`不同服务器上的相同内容

- 2.让每个用户从`最靠近`的服务器访问内容

IP任播广泛应用于DNS，用于将DNS请求指向最近的根DNS服务器

在IP任播的配置阶段，为多台服务器指派`相同的IP地址`，并通告给BGP路由器，当配置路由算选择表时，每台路由器将本地化使用BGP路由选择算法来挑选到该IP地址的最好的路由

当用户发起请求时，将收到所有服务器所使用的`共同IP地址`，当向这个IP地址发起请求时，`路由器`将向最近的服务器转发分组

### 路由选择策略

如果x是一个客户网络，它不希望其他AS的分组经由它而转发，做法是`不向它的邻居通告`所知晓的外界路径

而对一个网络供应商ISP来说，它可能不想转发其他ISP所管辖的子网络中的分组，一个普遍的法则是：任何穿越某ISP主干网的流量必须是其`源或目的`位于该ISP的某个客户网络中

## SDN控制平面

SDN体系结构有4个关键特征：

- 1.基于流的转发，SDN控制的网络交换机可以基于运输层、网络层或链路层首部进行转发，比如OpenFlow

- 2.控制平面与数据平面分离

- 3.网络控制功能，SDN的软件在`远程服务器`上执行，有两个组件：一个`SDN控制器`以及若干个`网络控制应用程序`
...
- 1.通信层：SDN控制器和受控网络设备之间的通信，设备需要能够向控制器传递本地观察到的时间，控制器和受控设备之间的接口称为`南向接口`

- 2.网络范围状态管理层

- 3.对于网络控制应用程序层的接口：称为`北向接口`

### OpenFLow协议

OpenFlow协议位于控制器的`通信层`中，运行在TCP上，使用6653作为默认端口号

从控制器到受控设备主要有以下报文：

- 1.配置，允许控制器查询并设置受控设备的参数

- 2.修改状态：修改交换机流表中的表项

- 3.读状态：用于从交换机的流表和端口收集数据

- 4.发送分组：用于在受控交换机从特定的端口发送出一个特定的报文
...
ICMP被主机和路由器用来彼此沟通`网络层`的信息，ICMP最典型的用途是`差错报告`

ICMP运行在IP之上，与TCP和UDP类似，ICMP报文是用作IP的`有效载荷（数据）`承载的，当一台主机收到指明上层协议为ICMP的IP数据报时（上层协议字段为`1`），它解出数据报的内容给ICMP

ICMP报文包含一个`类型字段`和一个`编码字段`，以及引起该ICMP报文的IP数据报的`首部和前8个字节`（详细的类型与编码表查看书P273）

ping程序就是基于ICMP协议的，它发送一个`类型8编码0`的报文到指定主机，目的主机回应一个`类型0编码0`的回答

## 网络管理和SNMP

### 网络管理的关键组件

- 1.管理服务器，它是一个应用程序，是执行网络管理活动的地方，控制网络管理信息的收集、处理、分析和显示，人类管理员在此与网络设备打交道

- 2.被管设备，可以使主机、路由器、交换机、中间盒等，每个被管设备中，有几个`被管对象`，比如硬件组件或软件组件的配置参数

- 3.管理信息库，被管设备的每个被管对象的关联信息手机在管理信息库中

- 4.网络管理代理，是被管对象中的一个`进程`，用于与管理服务器通信

- 5.网络管理协议

### 简单网络管理协议 SNMP

简单网络管理协议（Simple Network Management Protocal）是一个`应用层协议`，用于在管理服务器和代表管理服务器执行的代理之间传递网络控制和信息报文

SNMP最常使用的是`请求响应`模式，由SNMP管理服务器向SNMP代理发送请求，`代理接收`到请求后，执行某些动作，然后响应该请求

还有一种常用的用法是代理向管理服务器发送一种非请求报文，称为`陷阱报文`，用于告知管理服务器一个异常情况已经引起了管理信息库的改变

SNMP PDU（协议数据单元）通常是作为`UDP数据报`的载荷进行传输的


# 链路层和局域网

链路层中的设备称为`节点`，包括主机、路由器、交换机和WiFi接入点，连接相邻节点的通信信道称为`链路`，链路层的分组称为`链路层帧`

## 链路层概述

### 链路层提供的服务

- 1.成帧，在网络层数据报经链路传送之前，链路层协议需要先将其用链路层帧封装起来

- 2.链路接入，`媒体访问控制（Medium Access Control MAC）协议`规定了帧在链路上传输的规则

- 3.可靠交付，链路层提供`可靠交付`服务

- 4.差错检测和纠正

### 链路层在何处实现

链路层的主体部分是在`网络适配器`中实现的，或称为`网络接口卡`，网络适配器的核心是`链路层控制器`，大部分是通过`硬件`实现的

链路层是硬件和软件交接的地方

## 差错检测和纠正技术

在发送节点，使用`差错检测和纠正比特（Error-Detection and-Correctoin EDC）`来增强数据D

检测差错的三种常用技术：`奇偶校验`、`检验和`以及`循环冗余检测`

### 奇偶校验

最简单的方式是使用单个`奇偶校验位`，假设要发送的信息D有d比特，在偶校验中，发送方只需包含一个附加的比特，使得d+1比特中`1的个数是偶数`，奇校验则使得1的个数是奇数，接收方只需要统计1的个数即可

但是如果同时发生多个差错，单个奇偶校验位可能不足以检测出差错

解决方法是将单比特奇偶校验`二维化`，将D中的d个比特划分为i行j列，对每行每列计算奇偶值，所以共有`i+j+1位`奇偶比特

采用二维奇偶校验时，如果第(i, j)个比特发生了变化，那么第i行和第j列的奇偶校验位都会发生变化，能够准确定位到发生差错的比特并`纠正`它，如果多个比特发生了差错，二维奇偶校验也能检测出来，但`不能纠正`

这种接收方检测和纠正差错的能力被称为`前向纠错`

### 检验和方法

在检验和奇数中，d比特的数据被划分为一个k比特整数的`序列`，简单的校验和方法就是将k比特整数加起来，将得到的`和`作为差错检测比特

### 循环冗余检测

循环冗余检测（Cyclic Redundancy Check CRC）编码，也成为`多项式编码`

使用CRC编码，发送方和接受方时首先需要协商一个r+1比特模式，称为`生成多项式`，表示为G，然后发送方要选择`r个附加比特R`，将它们附加到D上，使得得到的d+r比特用`模2算术`恰好能`被G整除`

模2算术包括加减乘除，是一种`不进位、不借位`的算法，所以模2算数中的加减法等价于`按位异或XOR`

对于给定的D，d+r比特模式为`D * 2^r XOR R`，关键问题在于如何计算出R
...
因为XOR等价于模2的加减法，所以如果用G来除D*2^r，余数就是R：
...
每个CRC标准都能检测出`小于r+1比特`的突发差错，以及任何`奇数个`比特差错

## 多路访问链路和协议

网络链路有两种：`点对点链路`和`广播链路`

点对点链路由单个发送方和单个接收方组成

而在广播链路中，多个发送方和接收方都连接到了相同的、单一的、共享的`广播信道`上，`多路访问问题`指的就是如何协调多个发送节点和接收节点对一个共享广播信道的访问

多路房屋内协议可划分为三种类型：`信道划分协议`、`随机接入协议`和`轮流协议`

### 信道划分协议

常用的信道划分方式有`时分多路复用（TDM）`和`频分多路复用（FDM）`

- TDM

TDM将时间划分为时间帧，并将每个时间帧划分为N个时隙，然后将每个时隙分配给N个节点中的一个，每个节点只能在它`被指定的时隙`传输分组

每个节点传输速率为R/N bps，而且就算仅有一个节点传输分组时，它也需要等待自己的时隙

- FDM

FDM将R bps信道划分为不同的频段，并将每个频段分给N个节点中的一个，每个节点的传输速率也为R/N bps

- CDMA

除了TDM和FDM之外，还有一种划分协议称为`码分多址（Code Division Multiple Access CDMA）`

CDMA为每个节点分配一种不同的`编码`，所有的节点可以`同时传输`，在接收方根据约定的编码来得到相应的数据

### 随机接入协议

在随机接入协议中，每个节点都以信道的`全部速率`进行传输，当发生碰撞时，涉及碰撞的节点重传分组

不过，重传分组`并不立即执行`，而是先等待一个`随机时延`，每个节点都独立的选择时延

#### 时隙ALOHA

在最简单的时隙ALOHA协议中，令p是一个概率，当出现碰撞时，节点以`概率p`在后续的`每个时隙`中重传帧，直到它被无碰撞地传输出去

ALOHA的最大效率为`1/e=0.37`（详细说明请看书P296）

#### 载波侦听多路访问 CSMA

与ALOHA不同，在CSMA中每个节点不是无限制的传输自己的分组，而是要`监听`，即在传输前首先监听信道，等到`信道空闲`时才开始传输，同时还需要`碰撞检测`，即当一个节点传输分组时也在不停的监听信道，如果检测到另一个节点正在传输，它自己就`停止传输`

当出现碰撞后，停止传输并等待一个`随机时间`后再重新传输

以太网就是一种CSMA协议

### 轮流协议

ALOHA和CSMA协议能够做到在只有一个节点活跃时，具有R bps的吞吐量，但不能做到当有M个节点活跃时，每个节点的吞吐量为R/M bps，`轮流协议`可以同时实现这两点

轮流协议有两种比较重要的协议：`轮询协议`和`令牌传递协议`

- 轮询协议

将节点之一设置为`主节点`，主节点以循环的方式轮询每个节点，告诉它们能传输的节点的最多数量

- 令牌传递协议

没有主节点，而是一种被称为`令牌`的特殊帧在节点之间以某种`固定的次序`彼此交换，当一个节点收到令牌时，尽在它有一些帧需要发送时才持有令牌，否则立即向下一个节点转发令牌
...
主机或路由器的`适配器`具有链路层地址，链路层交换机不具有链路层地址，通常链路层地址称为MAC地址

MAC地址长度为`6字节`，通常使用十六进制表示法

一个适配器可以接收并非向他寻址的帧，它会检查帧中的MAC地址是否与自身`匹配`，如果匹配就提取出数据报向上传奇，不匹配就丢弃该帧

`MAC广播地址 FF-FF-FF-FF-FF-FF`可以被所有适配器接收并处理

#### 地址解析协议

地址解析协议（Address Resolution Protocal ARP）的任务是实现网络层地址和链路层地址之间的转换

每台主机和路由器在内存中有一个`ARP表`，包含了IP地址到MAC地址的映射关系，以及每个映射的`寿命（TTL）`

在同一个子网中，假设222.222.222.220主机要向222.222.222.222主机发送分组，流程为:

- 1.如果发送方的`ARP表`中已经包含了目的主机，则直接将目的主机的IP地址作为输入就可以查询出对应的MAC地址

- 2.如果没有包含，则首先发送方构建一个`ARP查询分组`，它包含目的IP地址，并使用广播MAC地址想子网中所有适配器发送，然后接收适配器将ARP分组传递给`ARP模块`，ARP模块将检查自身的IP地址是否与ARP分组中的目的IP地址匹配，与之匹配则给查询主机返回一个`带有所希望映射`的`ARP响应分组`，查询主机就可以`更新`它的ARP表，然后可以正常传输

当要发送数据到子网之外，如222.222.222.220要想111.111.111.111发送分组，流程为：

- 1.首先按照子网中传递的方式，以`路由器`接口在当前子网中的MAC地址为目的地址发送

- 2.分组到达路由器后，路由器可以根据目的IP地址，通过`转发表`得到应该转发的输出接口（网络层操作）

- 3.在路由器另一端的子网中，按照子网发送方式将分组发送到目的主机
...
- 1.前同步码，8字节，前7个字节都是`10101010`，最后一个字节是`10101011`，前七字节用于`唤醒接受适配器`，并且将它的时钟与发送方`时钟同步`，第8字节的最后两个连续的1用于告诉接收适配器，有用信息即将从此处`开始`

- 2.目的地址，6字节，接收适配器的MAC地址

- 3.源地址，6字节，发送适配器的MAC地址

- 4.类型，2字节，指示上层的`网络层协议`

- 5.数据，承载IP数据报，以太网的最大传输单元（MTU）为`1500字节`，超过的数据需要分片，最小`46字节`，不足的数据需要填充

- 6.CRC，用于循环冗余检测的校验

以太网向网络层提供`无连接`、`不可靠`服务，即发送时不需要事先握手，接收方也不返回确认帧

#### 以太网技术

- 以太网是链路层也是物理层的规范

- 经过多年发展，现在基于交换机的以太局域网中不会有碰撞
...
`过滤`是决定一个帧应该转发到某个接口还是应该将其丢弃的交换机功能，`转发`是决定一个只能应该被导向哪个接口

转发和过滤功能基于`交换机表`完成，它有三个表项：
...
- 1.表中没有`目的地址`的表项，交换机向除了x的所有接口的`接收缓存`转发该帧的`副本`

- 2.目的地址与x相关联，则执行过滤功能将该帧丢弃

- 3.目的地址与一个x之外的某个接口关联，则执行转发功能将该帧转发到该接口的输入缓存

#### 自学习

交换机表是自动、动态和自治地建立的，表现在：表初始为空，当出现一个新的MAC地址，将其存入表中，并在超过一定时间后删除表项

#### 链路层交换机的性质

- 1.消除碰撞：交换机能够缓存帧并不会在同时时刻传输多个帧

- 2.异质的链路：交换机将链路彼此隔离，所以局域网中的不同链路能够以`不同的速率`并在`不同的媒体`上运行

- 3.管理：交换机能够检测到适配器的异常并断开相应的链路

#### 交换机与路由器对比

路由器可以至高运行在网络层，交换机至高在链路层

路由器不是即插即用的，需要人为配置IP地址

### 虚拟局域网

普通的物理局域网有几点不足之处：缺乏流量隔离、交换机的接口浪费以及难以频繁更改用户信息

通过`虚拟局域网（VLAN）`可以解决这些问题，支持VLAN的交换机可以将单一的物理设备定义成多个虚拟局域网

在VLAN中，交换机的端口被划分成组，每个组构成一个VLAN，在每个VLAN中的端口形成一个`广播域`，来自一个端口的广播流量只能到达该广播域中的其他端口

为了在不同的VLAN之间通信，将每个VLAN的一个端口域一台`外部路由器`相连，输出的分组先经过外部路由器，在到达接收主机所在的VLAN

为了能够识别VLAN，在基于VLAN通信的链路层帧的首部加入了`4字节`的`VLAN标签`
...
- 1.对称密钥系统：发送方A和接收方B的密钥是`相同的`，且是`秘密的`

- 2.公开密钥系统：有`一对`密钥，一个是公开为`所有人`所知的，另一个只有A或B`一个人知道`

通常对称密钥算法（比如DES）要比公开密钥算法（RSA）如快得多，所以可以使用公开密钥算法创建一个`对称的会话密钥`，后续的传输使用该会话密钥加密

## 报文完整性和数字签名

### 密码散列函数

散列函数的目的是使得H(x) = H(y)，即两个不同的报文经相同的散列函数计算出的结果计算上不可能相等
...
- 1.A生成报文m，用s级联m生成m+s，并计算散列H(m+s)，H(m+s)称为`报文鉴别码（Message Authentication Code MAC）`

- 2.A将MAC附加到报文上，生成扩展报文(m, H(m+s))发送给B

- 3.B接收到扩展报文(m, h)，B也知道s，它可以计算出MAC，如果H(m+s) = h，则传输正常

散列不是为了要恢复报文的，而是要检验报文是否和原来的一致

### 数字签名（鉴别）

数字签名是为了指出一个文件的所有者或创作者，或者表明某人认可一个文件内容

数字签名应该以`可鉴别`的、`不可伪造`的方式进行

常用的方法是使用`私钥`对`报文的散列`进行加密，流程如下：

- 1.A先为报文m生成散列H(m)，然后用私钥进行加密得到Ka_(H(m))，将(m, Ka_(H(m)))发给B

- 2.B首先用A的公钥计算出散列码H(m)=Ka(Ka_(H(m)))，然后用散列码验证报文

#### 公钥认证

接收方B不能判断发送方A所声称的公钥就是A实际上的公钥（也有可能是C冒充A发送的）

所以需要公钥认证，通常是由`认证中心（Certification Authority CA）`完成的

CA能够验证某个实体的身份，并生成一个将其身份和实体的公钥绑定的`证书`，证书包含`公钥`和公钥所有者全局唯一的`身份标识`，并由CA对证书进行`数字签名`

接收方B使用`CA的公钥`来核对A的证书并提取出A的公钥，这样就可以认定A的公钥是代表A的

## SSL

`SSL（Secure Socket Layer）安全套接字层`是TCP的强化版本，SSL有一个稍加修改的版本，称为`运输层安全性（Transport Later Security TLS）`

SSL技术上位于应用层，但可以视为在运输层和应用层之间

### 简化描述

简单描述，SSL具有三个阶段：握手、密钥到处和数据传输，假设A是服务器而B是客户

#### 1.握手

- 1.B与A创建一条TCP连接，向A发送一个SSL hello报文

- 2.A用它的`证书`进行响应，证书中包含了它的公钥

- 3.B产生一个`主密钥MS`，用A的公钥加密生成加密的`主密钥EMS`，发送给A，这个主密钥将作为在A与B通信时的`对称密钥`
...
SSL将数据流分割成`记录`，为每个记录附加一个MAC用于完整性检查，首先通过H(记录+MB)得到报文鉴别码，然后使用EB加密(记录+H(记录+MB))，发送给A

为了防止中途TCP报文段的序号被篡改（序号未被加密），B还需要维护一个`序号计数器`，每发送一个SSL加1，计算MAC时，把序号`包含在MAC中`

## 网络层安全性：IPsec和虚拟专用网

IPsec是`IP安全（IP Security）协议`，它为网络层实体间的IP数据报提供了安全

许多机构使用IPsec创建了因特网上`虚拟专用网（Virtual Private Network VPN）`

使用VPN可以在公共因特网中创建专有网络，而不必花费巨大的开销去建立物理网络设施，为了提供机密性，专用网络的数据在进入公共因特网之前进行`加密`，将IPv4
转换成IPsec数据报

IPsec通过`隧道`技术封装IPv4数据报

## 防火墙

防火墙能够分成3类：传统分组过滤器、状态过滤器和应用程序网关

### 传统分组过滤器

在一个网络组织的`网关路由器`中进行分组过滤

通过在路由器中使用`访问控制列表`实现防火墙规则，过滤的规则可以自行设置

### 状态过滤器

除了访问控制列表外，状态过滤器还使用一个`连接表`来跟踪所有进行中的TCP连接

### 应用程序网关

应用程序网关是一个应用程序（进程）特定的服务器
...
多媒体应用可分为：`流式存储音频/视频`、`会话式IP音频/视频`和`流式实况音频/视频`等
...
- 2.时域冗余：反映一幅图像和后续图像的重复程度

根据压缩可以生成视频的不同质量等级的版本

### 音频

模拟音频信号首先以某种固定速率`采样`，然后采样值被四舍五入`量化`为有限个数值中的一个，且每个量化值由固定数量的比特标识，通过音频扬声器将数字信号转换为模拟信号播放，这中间会损失声音质量

这种编码技术称为`脉冲编码调制（PCM）`，采样速率为每秒8000个样本，每个样本用8比特表示

PCM编码的音频需要被压缩，通常采用的技术是`MPEG1第3层`，也就是MP3

## 流式存储视频

在流式存储视频中，需要将视频事先录制好，而当客户端在视频的一个位置开始播放时，同时也正在从服务器接收视频的后续部分，这种技术称为`流`

流式视频最重要的测度是`平均吞吐量`，它需要大于流视频本身的比特率

流式视频可分为三种类型：`UDP流`、`HTTP流`和`适应性HTTP流`

无论哪种类型都应用了`客户缓存`

### UDP流

UDP流使用`实时传输协议（Real-Time Transport Protocol RTP）`

除了服务器到客户的视频流外，两者之间还并行维护一条`控制连接`

### HTTP流

视频作为一个具有特定`URL`的普通文件存储在HTTP服务器上，客户端发起一个HTTP GET请求来访问视频文件

## IP语音

IP语音（Voice-over-IP VoIP）也被称为因特网电话

### 尽力而为服务的限制

实时会话应用对分组时延、时延抖动和丢包非常敏感

加强VoIP性能主要在`应用层`技术上

为了消除时延抖动，可以使用`序号`、`时间戳`和`播放时延`

为了在丢包情况下保护数据，有两种丢包预期方案：`前向纠错 FEC`与`交织`

- FEC的基本思想是给初始的分组增加`冗余信息`

- 交织方式在发送时发送一个`重新排列`的音频交织流

# 支持多媒体的网络

对多媒体网络应用提供网络层支持有三种方法：`尽力而为服务`、`区分服务`和`每连接服务质量（QoS）保证`
